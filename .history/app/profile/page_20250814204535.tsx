...
const ProfilePage = () => {
  ...
  // Redirect if not authenticated (after auth initialized)
  useEffect(() => {
  ...
  if (!initialized) {
    // Show loader or nothing until auth is initialized
    ...
    <section className="min-h-screen main-max-width padding-x mx-auto my-10">
      ...
      {activeTab === "orders" && (
        <div role="tabpanel" aria-labelledby="orders-tab">
          ...
          {/* Optionally add empty state if no orders */}
          {/* Assuming PurchasedOrder shows orders internally */}
        </div>
      )}
